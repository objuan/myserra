...
bool ESP8266::sATCWMODE(uint8_t mode)
{
    ...
    data = recvString(F("OK"), F("no change"));
    if (data.indexOf(F("OK")) != -1 || data.indexOf(F("no change")) != -1) {
        return true;
    }
    return false;
}
...
bool ESP8266::sATCWSAP(String ssid, String pwd, uint8_t chl, uint8_t ecn)
{
    ...
    data = recvString(F("OK"), F("ERROR"), 5000);
    if (data.indexOf(F("OK")) != -1) {
        return true;
    }
    return false;
...
bool ESP8266::sATCIPSTARTSingle(String type, String addr, uint32_t port)
{
    ...
    data = recvString(F("OK"), F("ERROR"), F("ALREADY CONNECT"), 10000);
...
bool ESP8266::sATCIPMUX(uint8_t mode)
{
    ...
    data = recvString(F("OK"), F("Link is builded"));
    if (data.indexOf(F("OK")) != -1) {
        return true;
    }
    return false;
